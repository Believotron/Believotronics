...
def sample_first_joystick():
    ...
    def on_button(button, pressed):
        ...
        if   (button == 10):
            ...
            positionX -= 1
            print("Left %d" % positionX)
            command = "G1 X" + str(positionX)
            #command = "G1 X" + str(-50)
            print(command)
            p.send_now(command)
            #p.send_now("G1 X%d" % positionX )
            #p.send_now("G1 X-50")
        elif ( (button == 4) and (pressed == 1) ):
            positionX +=1
            command = "G1 X" + str(positionX)
            print("Right %d" % positionX)
            print(command)
            #command = "G1 X" + str(50)
            p.send_now(command)
            #p.send_now("G1 X%d" % positionX )
